...
export const floyd_steinberg_dither = (
  source: ImageData,
  palette: string[]
): ImageData => {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      distribute_error(data, x + 1, y, width, height, error_r, error_g, error_b, 7 / 16);
      distribute_error(data, x - 1, y + 1, width, height, error_r, error_g, error_b, 3 / 16);
      distribute_error(data, x, y + 1, width, height, error_r, error_g, error_b, 5 / 16);
      distribute_error(data, x + 1, y + 1, width, height, error_r, error_g, error_b, 1 / 16);
    }
  }
